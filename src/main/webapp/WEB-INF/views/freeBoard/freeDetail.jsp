...
<body>
		...
		<div class="container">
			<div class="row">
				<div class="col-xs-12 col-md-9 write-wrap">
					...
					<button class="form-control">더보기(페이징)</button>
				</div>
			</div>
		</div>
	</section>
	...
	$(document).ready(function(){
		...
		function add() {
			...
			//중단
				console.log({"bno":bno, "reply":reply, "replyId": replyId, "replyPw":replyPw});//문자열의 형태로 변경해서 요청
			$.ajax({
				type : "POST",//요청방식
				url:"../reply/replyRegist",
				data: JSON.stringify({"bno":bno, "reply":reply, "replyId": replyId, "replyPw":replyPw}),//제이슨형식으로 보낼수도 있고, 폼형식으로 가능,
				dataType : "text" , //서버로 부터 어떤형식으로 받을건지 
				contentType :"application/json; charset=utf-8", //기본값ㄷ은 폼형식을 지칭하는데 ,JSON형식으로 보낼때는 반드시 타입을 명시해야함
				success : function(data){ //요청성공시 돌려받을 콜백함수
					if(data == 1){
						$("#reply").val("");
						$("#replyId").val("");
						$("#replyPw").val("");
						getList(1, true);//목록메서드 호출
					}else {
						alert("등록에 실패했습니다. 잠시후에 다시 시도 하세요");
					}
					//console.log(data)
		...
		//더보기 버튼처리(클릭시 전역변수 페이지 번호에 =1 값을 전달)
		...
		var page = 1;//페이지번호
		var strAdd = ""; //화면에 그려넣을 태그를 문자열의 형태로 추가할 함수
		getList(1, true);//상세화면 진입시에 리스트 목록을 가져옴
		function getList(pageNum, reset){
		
			var bno = "${boardVO.bno}"; //게시글번호
			$.getJSON("../reply/getList/" + bno +"/"+ pageNum,
					  function(data){
					console.log(data);
					...
					for(var i = 0; i <data.length; i++){
					 ...
					 strAdd += "<small class='left'>"+timeStamp(data[i].replyDate)+"</small>" + data[i].rno;		                               
					...
					//화면에 그릴때 reply-wrap을 disply:none으로 선언하고, 제이쿼리 fadein함수로 서서히 보이게 처리
					$(".reply-wrap").fadeIn(500);
			})   //$.getJSON({요청주소, 성공시콜백함수})
		}//getList 함수 end
		
		//수정삭제
		/*
		에이잭스의 실행이 더 늦게 완료가 되므로, 실제 이벤트의 선언이 먼저 실행되게 됩니다.
		그러면 화면에 댓글관련 창은 아무것도 등록되지 않은 형태이므로, 일반 클릭 이벤트가 동닥하지 않습니다.
		...
		*/
		$("#replyList").on("click","a",function(){
			...
			// hasClass는 매개값이 포함되있으면 true 아니면 false를 반환하는 클래스 확인 함수입니다.
			...
			if( $(this).hasClass("replyModify")){
				...
				$.ajax({
					...
					contentType: "application/jsno; charset=utf-8",
					success: function(data){
						...
						if(data == 1){//업데이트 성공
							...
							alert("수정에 실패했습니다");
							//$("#modalReply").val("");//수정창비우기
							$("#modalPw").val(""); //비밀번호 창 비우기
						}
						
					},
					error: function(status, error){
						console.log("수정에 실패했습니다 관리자에게 문의하세요");
					}
				})
				//$("#modalRno").modal("show"); //모달창에 히든태그에 댓글의 번호를 세팅
			}else{
				//삭제 모달형식으로 변경
		...
		$.ajax({
			...
			if(data == 1){
				...
				$("#modalPw").val(""); 
				$("#replyModal").modal("hide"); //모달창 내리기(부트스트랩제공)
				getList(); //리스트호출
			}else{
				alert("수정에실패했습니다.");
				$("#modalPw").val(""); //비밀번호만 지우기
			}
		 ...
		 */
			...
			2. ajax함수를 이용해서 POST방식으로 reply/delete 요청, 필요한 값은 JSON형식으로처리해서 요청
			3. 서버에서는 요청을 받아서 비밀번호를 확인하고, 비밀번호가 맞다면 업데이트를 진행하면 됩니다.
			4. 만약 비밀번호가 틀렸다면, 0을 반화해서 비밀번호가 틀렸습니다. 경고창르 띄우세요
			5. 업데이트가 진행된 다음에는 modal창의 값을 공백으로 초기회 시키세요
			*/
			...
			$("#modalDelBtn").click(function(){
				...
				$.ajax({
					...
					error:function(status, error){
						console.log("삭제에 실패했습니다. 관리자에게 문의하세요");
					}
